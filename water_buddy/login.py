# app_with_mascots.py
# Full Streamlit app (your original functionality) with mascots integrated inside page layout.
# - Keeps your Gemini setup (from st.secrets / .env) and SQLite persistence
# - Mascots appear INSIDE pages (not floating notifications)
# - Mascot image files loaded from your GitHub raw assets folder
# - Mascot messages generated by Gemini when available; fallbacks used on errors
# - Mascots shown only on pages/times specified (no mascot on Report page)
#
# Drop this file in place of your old app file and run with Streamlit.
...
# -------------------------------
# Helper functions around user data and weeks/days
# -------------------------------
...
def choose_mascot_and_message(page: str, username: str) -> Optional[Dict[str, str]]:
    ...
    if page == "home":
        ...
        # Meal windows: Breakfast 8–9 AM, Lunch 13–14, Dinner 20:30–21:30
        ...
        # Reminder window (5 min before/after)
...
def render_mascot_inline(mascot: Dict[str, str], position: str = "below_header"):
    """
    Render the mascot inline in the page content.
    position param reserved for potential layout tweaks (not strictly used).
    """
    if not mascot:
        return
    ...
    # Use columns: small left column for image, larger right for message
    col_img, col_msg = st.columns([1, 4])
    with col_img:
        try:
            st.image(img, width=90)
        except Exception:
            # Fallback: show small placeholder if image fails
...
# -------------------------------
    ...
    st.markdown("<h1 style='text-align:left; color:#1A73E8;'>💧 HP PARTNER</h1>", unsafe_allow_html=True)
    
    fill_percent = min(st.session_state.total_intake / daily_goal, 1.0) if daily_goal > 0 else 0
    
    bottle_html = f"""
    <div style='width: 140px; height: 360px; border: 3px solid #1A73E8; border-radius: 20px; position: relative; margin-left:40px; 
    background: linear-gradient(to top, #1A73E8 {fill_percent*100}%, #E0E0E0 {fill_percent*100}%);'>
        <div style='position: absolute; bottom: 10px; width: 100%; text-align: center; color: #fff; font-weight: bold; font-size: 18px;'>
            {round(st.session_state.total_intake,2)}L / {daily_goal}L
        </div>
    </div>
    """
    # Layout: left column large bottle + contents; right column for controls and mascot below header
    ...
    with right_col:
        ...
        # Chatbot toggle
        chat_button_clicked = st.button("🤖 Chat", key="chat_button_home", help="Chat with Water Buddy")
        ...
        if st.session_state.show_chatbot:
            ...
            for entry in st.session_state.chat_history:
                if entry["sender"] == "bot":
                    st.markdown(f"<div style='background:#F1F1F1; padding:8px; border-radius:10px; margin-bottom:6px;'>🤖 {entry['text']}</div>", unsafe_allow_html=True)
            user_msg = st.text_input("Type your message...", key="chat_input_home")
            if st.button("Send", key="send_btn_home"):
                if user_msg.strip():
                    try:
                        if model:
    ...
    # Mascot inline on Home page (below header area)
...
# -------------------------------
    ...
    # No mascot on Report page by design (clean view)
...
# -------------------------------
    ...
    st.markdown(f"<h2 style='text-align:left; color:#1A73E8;'>🔥 Daily Streak: {current_streak} Days</h2>", unsafe_allow_html=True)
    st.write("---")
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        if st.button("🏠 Home"):
...
# Note: keep the DB connection open for the app lifetime. If you wish to close:
# conn.close()
